...
export function latinize(str: string): string {
  ...
  return str.replace(/[^A-Za-z0-9\[\] ]/g, (a: string): string => {
    return latinMap[a] || a;
  });
}
...
export function tokenize(str: string, wordRegexDelimiters = " ", phraseRegexDelimiters = ""): Array<string> {
  const regexStr: string = "(?:[" + phraseRegexDelimiters + "])([^"
    + phraseRegexDelimiters + "]+)(?:[" + phraseRegexDelimiters + "])|([^"
    + wordRegexDelimiters + "]+)";
  const preTokenized: Array<string> = str.split(new RegExp(regexStr, "g"));
...
export function getValueFromObject(object: any, option: string): string {
  ...
  const properties: string = option.replace(/\[(\w+)\]/g, ".$1")
    .replace(/^\./, "");
  const propertiesArray: Array<string> = properties.split(".");
